...
$logFilePath = "C:\BrockIT\PS scripts\MailboxTrafficReport\mailbox.log"
...
try {
    ...
    if ($messageTrace -and $messageTrace.Count -gt 0) {
        ...
        if ($outgoingEmailsPerUser -and $outgoingEmailsPerUser.Count -gt 0) {
            ...
            $results | Export-Csv -Path "C:\BrockIT\PS scripts\MailboxTrafficReport\mailbox.csv" -NoTypeInformation
